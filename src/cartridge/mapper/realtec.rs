...
impl Cartridge {
    ...
    pub(in crate::cartridge) fn read_prg_mapper236(&self, addr: u16) -> u8 {
        ...
        let bank = if self.mapper236_chr_ram {
            let outer_base = (self.mapper236_outer_bank as usize) * 8;
            let inner_bank = (self.prg_bank as usize) & 0x07;

            match self.mapper236_mode & 0x03 {
                0 | 1 => {
                    if upper_half != 0 {
                        outer_base + 7
                    } else {
    ...
    pub(in crate::cartridge) fn write_prg_mapper236(&mut self, addr: u16, _data: u8) {
        match addr & 0xC000 {
            0x8000 => {
                ...
                if self.mapper236_chr_ram {
                    self.mapper236_outer_bank = (addr & 0x0007) as u8;
                } else {
                    self.chr_bank = (addr & 0x000F) as u8;
                }
            }
            0xC000 => {
                self.mapper236_mode = ((addr >> 4) & 0x03) as u8;
                self.prg_bank = if self.mapper236_chr_ram {
                    (addr & 0x0007) as u8
                } else {
                    (addr & 0x000F) as u8
                };
