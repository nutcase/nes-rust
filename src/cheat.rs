pub const WORK_RAM_SIZE: usize = 0x800;
...
#[derive(Clone)]
#[cfg_attr(feature = "cheat-ui", derive(serde::Serialize, serde::Deserialize))]
...
impl CheatManager {
    ...
    /// Apply cheats to a combined RAM buffer (cpu_ram ++ prg_ram).
...
mod tests {
    ...
    fn test_search_with_extended_ram() {
        // Simulate cpu_ram (2KB) + prg_ram (8KB)
        let size = WORK_RAM_SIZE + 0x2000;
        let mut ram = vec![0u8; size];
        ram[0x900] = 42; // In prg_ram region
