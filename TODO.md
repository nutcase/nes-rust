...
- ✅ **自動入力機能削除**: テスト用の自動押下（AUTO_INPUT_*）は撤去し、手動入力に統一
...
14. **自動入力（AUTO_INPUT_*）削除** (`src/emulator.rs`)
   - 以前のデバッグ用自動押下は、意図せずテストが進む原因になったため撤去
   - 以降は手動操作での確認に統一（必要なら外部ツールで入力を与える）
...
#### 1. パレット問題の調査結果（調査完了）
**結果**:
- ✅ **自動入力（AUTO_INPUT_*）削除**: 意図しない進行を防ぐため撤去
- ✅ **長時間テスト完了**: 2000フレーム（約33秒）実行
